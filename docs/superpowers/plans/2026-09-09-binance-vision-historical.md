...
- [ ] Write failing tests for URL generation, header/no-header Binance kline parsing, monthly-first selection, daily fallback, date filtering, cache reuse, and invalid periods.
- [ ] Run `pytest -q tests/test_binance_vision_data_source.py` and confirm failure because the source is not present.
- [ ] Commit only the tests if the repository workflow requires task-sized commits.
...
- [ ] Implement UTC period parsing and validation.
- [ ] Implement monthly archive URL resolution and daily fallback on not-found responses only.
- [ ] Implement deterministic cache paths and ZIP member selection.
- [ ] Implement parsing for both headered and headerless archives and normalize/sort/filter frames.
- [ ] Run source tests and verify they pass.
...
- [ ] Write failing tests for default Vision source selection, required period validation, configured symbols/intervals propagation, and unchanged `--backtest`/`--live` flags.
- [ ] Run the focused tests and confirm expected failures.
- [ ] Implement source selection and pass all configured intervals to Vision while retaining the first interval as execution interval.
- [ ] Preserve credential-backed strategy/exchange metadata initialization without using REST candle history.
- [ ] Run focused runner and CLI tests.
...
- [ ] Document the Vision URL layout, cache behavior, date semantics, CLI examples, metadata credential behavior, and explicit legacy REST source status.
- [ ] Run the full package test suite, changed-file lint, type check, import check, and a synthetic Vision-source backtest.
