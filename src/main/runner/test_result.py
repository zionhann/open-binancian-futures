...
from model.position import PositionList
...
class TestResult:
    ...
    def evaluate_filled_orders(
        ...
        for order in orders.filled_orders_backtesting(high, low):
            if order.is_type(OrderType.LIMIT) and positions.is_empty():
                positions.open_position_backtesting(balance, order, time, leverage)
